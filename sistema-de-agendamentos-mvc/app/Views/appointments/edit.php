<?php $title = 'Editar Agendamento – SchedMVC'; ?>
...
<div class="min-h-screen bg-slate-50 flex flex-col">
  ...
  <header class="h-14 bg-white border-b border-slate-200 flex items-center px-6 gap-4 shrink-0">
    ...
    <h1 class="text-base font-bold text-slate-800">Editar Agendamento</h1>
  </header>
  ...
  <div class="flex-1 flex items-start justify-center p-6">
    <div class="w-full max-w-lg">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
        <form method="POST" action="/appointments/<?= $record['id'] ?>/update" class="space-y-5">
          ...
          <div>
            ...
            <select name="type" id="typeSelect"
              ...
              <option value="Standard"     <?= $record['type'] === 'Standard'     ? 'selected' : '' ?>>Padrão</option>
              <option value="Urgent"       <?= $record['type'] === 'Urgent'       ? 'selected' : '' ?>>Urgente</option>
              <option value="Professional" <?= $record['type'] === 'Professional' ? 'selected' : '' ?>>Profissional / Comercial</option>
            </select>
          </div>
          ...
          <div>
            ...
            <textarea name="description" rows="3"
              class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"><?= htmlspecialchars($record['description'] ?? '') ?></textarea>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="time" name="time"
                value="<?= htmlspecialchars(substr($record['time'] ?? '', 0, 5)) ?>"
                class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
          </div>
          ...
          <div id="fieldPriority" class="hidden">
            <label class="block text-sm font-semibold text-slate-700 mb-1">Nível de Prioridade</label>
            <select name="priority_level"
              class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="Alta"      <?= ($record['priority_level'] ?? '') === 'Alta'      ? 'selected' : '' ?>>Alta</option>
              <option value="Altíssima" <?= ($record['priority_level'] ?? '') === 'Altíssima' ? 'selected' : '' ?>>Altíssima</option>
            </select>
          </div>

          <!-- Campo exclusivo Profissional -->
          <div id="fieldClient" class="hidden">
            <label class="block text-sm font-semibold text-slate-700 mb-1">Nome do Cliente / Paciente</label>
            <input type="text" name="client_name"
              value="<?= htmlspecialchars($record['client_name'] ?? '') ?>"
              class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>
